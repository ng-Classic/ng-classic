...
Other projects developed by the Angular Classicteam like angular-cli, Angular ClassicMaterial, will be covered by these or similar guarantees in the future as they mature.
...
Our peer dependencies (such as TypeScript, Zone.js, or RxJS) are not considered part of our API surface, but they are included in our SemVer policies. We might update the required version of any of these dependencies in minor releases if the update doesn't cause breaking changes for Angular Classicapplications. Peer dependency updates that result in non-trivial breaking changes must be deferred to major Angular Classicreleases.

<a name="final-classes"></a>

## Extending Angular Classicclasses
...
Angular Classictracks the status of the public API in a *golden file*, maintained with a tool called the *public API guard*.
...
(Read more about building Angular Classicwith Bazel [here](./BAZEL.md).)
