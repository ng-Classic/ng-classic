...
import {absoluteFrom} from '../../lib/ngtsc/file_system';
import {runInEachFileSystem} from '../../lib/ngtsc/file_system/testing';
import {loadStandardTestFiles} from '../../lib/ngtsc/testing';
import {restoreTypeScriptVersionForTesting, setTypeScriptVersionForTesting} from '../../lib/typescript_support';
...
function allTests(os: string) {
  describe('ngtsc behavioral tests', () => {
    ...
    describe('undecorated classes using Angular Classic features', () => {
        ...
        it('should give a compile-time error when a derived Directive inherits from a non-decorated class', () => {
          ...
          expect(diags[0].messageText)
              .toEqual(
                  ...
                  `does not have an Angular Classic decorator of its own. Dependency injection will not be able to resolve ` +
  ...
  `);
      ...
      it('should provide a hint when importing an invalid NgModule from node_modules', () => {
        ...
        verifyThrownError(
            ...
            'This likely means that the library (external) which declares NotAModule is not ' +
                'compatible with Angular Classic Ivy.');
      });
    ...
    describe('iframe processing', () => {
      ...
      it('should generate an attribute binding instruction with a validator function ' +
             '(making sure it\'s case-insensitive, since this is allowed in Angular Classic templates)',
         () => {
           env.write('test.ts', `
              import {Component} from '@angular-classic/core';
