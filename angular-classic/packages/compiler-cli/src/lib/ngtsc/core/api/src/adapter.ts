...
export interface NgCompilerAdapter extends
  ...
  /**
   * A path to a single file which represents the entrypoint of an Angular Classic Package Format library,
...
export interface SourceFileTypeIdentifier {
  /**
   * Distinguishes between shim files added by Angular Classic to the compilation process (both those
  ...
  /**
   * Distinguishes between resource files added by Angular Classic to the project and original files in the
   * user's program.
   *
