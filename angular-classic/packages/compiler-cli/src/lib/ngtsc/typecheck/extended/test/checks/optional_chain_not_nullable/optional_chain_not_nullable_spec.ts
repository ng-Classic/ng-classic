...
import {DiagnosticCategoryLabel} from '@angular-classic/compiler-cli/src/ngtsc/core/api';
...
import {ExtendedTemplateCheckerImpl} from '../../../lib/extended_template_checker';
...
runInEachFileSystem(() => {
  describe('OptionalChainNotNullableCheck', () => {
    it('binds the error code to its extended template diagnostic name', () => {
      ...
      expect(optionalChainNotNullableFactory.name)
          .toBe(ExtendedTemplateDiagnosticName.OPTIONAL_CHAIN_NOT_NULLABLE);
    });
    ...
    it('should not return a check if `strict` is enabled but `strictNullChecks` is disabled',
       () => {
         expect(optionalChainNotNullableFactory.create({strict: true, strictNullChecks: false}))
             .toBeNull();
       });
    ...
    it('should produce optional chain warning for property access', () => {
      ...
      expect(diags[0].messageText)
          .toContain(`the '?.' operator can be replaced with the '.' operator`);
