...
@description
You see this error when Angular Classicdetects an attribute binding or a property binding on an `<iframe>` element using the following property names:
...
To enforce that, Angular Classicrequires these attributes to be set on `<iframe>`s as
