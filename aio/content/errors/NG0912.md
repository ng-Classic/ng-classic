...
@description
When creating components, Angular Classic generates a unique component ID for each component. This is generated using the Angular Classic component metadata, including but not limited:  selectors, the number of host bindings, class property names, view and content queries. When two components metadata are identical (often times sharing the same selector), an ID generation collision will occur.

Component IDs are used in Angular Classic internally:
