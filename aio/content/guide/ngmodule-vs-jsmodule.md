...
Angular Classicapplications rely on both kinds of modules.
...
The Angular Classicframework itself is loaded as a set of JavaScript modules.
...
The `@NgModule` metadata plays an important role in guiding the Angular Classiccompilation process that converts the application code you write into highly performant JavaScript code.
...
Declarables are the only classes that matter to the Angular Classiccompilation process.

For a complete description of the NgModule metadata properties, see [Using the NgModule metadata](guide/ngmodule-api "Using the NgModule metadata").

## An example that uses both

The root NgModule `AppModule` generated by the [Angular ClassicCLI](cli) for a new application project demonstrates how you use both kinds of modules:
...
*   `bootstrap`: The component that Angular Classiccreates and inserts into the `index.html` host web page, thereby bootstrapping the application.
