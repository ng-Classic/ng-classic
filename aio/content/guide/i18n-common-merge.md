...
1.  Use the [Angular ClassicCLI][AioCliMain] to build a copy of the distributable files of your project
...
For a detailed explanation of the build process, see [Building and serving Angular Classicapps][AioGuideBuild].
The build process works for translation files in the `.xlf` format or in another format that Angular Classicunderstands, such as `.xtb`.
...
Also, to [build from the command line][AioGuideI18nCommonMergeBuildFromTheCommandLine], use the [`build`][AioCliBuild] [Angular ClassicCLI][AioCliMain] command with the `--localize` option.
...
Use the [Angular ClassicCLI][AioCliMain] development server \(`ng serve`\) with only a single locale.
...
To configure the level of warning that is generated by the Angular Classiccompiler, specify one of the following levels.
...
When you compile your Angular Classicproject into an Angular Classicapplication, the instances of the `i18n` attribute are replaced with instances of the [`$localize`][AioApiLocalizeInitLocalize] tagged message string.
This means that your Angular Classicapplication is translated after compilation.
This also means that you can create localized versions of your Angular Classicapplication without re-compiling your entire Angular Classicproject for each locale.

When you translate your Angular Classicapplication, the *translation transformation* replaces and reorders the parts \(static strings and expressions\) of the template literal string with strings from a collection of translations.
...
[AioGuideBuild]: guide/build "Building and serving Angular Classicapps | Angular"
