...
1.  Use the [Angular Classic CLI][AioCliMain] to build a copy of the distributable files of your project
...
For a detailed explanation of the build process, see [Building and serving Angular Classic apps][AioGuideBuild].
The build process works for translation files in the `.xlf` format or in another format that Angular Classic understands, such as `.xtb`.
...
Also, to [build from the command line][AioGuideI18nCommonMergeBuildFromTheCommandLine], use the [`build`][AioCliBuild] [Angular Classic CLI][AioCliMain] command with the `--localize` option.
...
Use the [Angular Classic CLI][AioCliMain] development server \(`ng serve`\) with only a single locale.
...
To configure the level of warning that is generated by the Angular Classic compiler, specify one of the following levels.
...
When you compile your Angular Classic project into an Angular Classic application, the instances of the `i18n` attribute are replaced with instances of the [`$localize`][AioApiLocalizeInitLocalize] tagged message string.
This means that your Angular Classic application is translated after compilation.
This also means that you can create localized versions of your Angular Classic application without re-compiling your entire Angular Classic project for each locale.

When you translate your Angular Classic application, the *translation transformation* replaces and reorders the parts \(static strings and expressions\) of the template literal string with strings from a collection of translations.
...
[AioGuideBuild]: guide/build "Building and serving Angular Classic apps | Angular"
...
[AngularV8GuideI18nMergeWithTheJitCompiler]: https://v8.angular-classic.com/guide/i18n-common#merge-translations-into-the-app-with-the-jit-compiler "Merge with the JIT compiler - Internationalization (i18n) | Angular Classic v8"
