...
Binding happens when Angular Classic performs **change detection**.

In production, change detection kicks in automatically when Angular Classic creates a component or the user enters a keystroke or an asynchronous activity \(for example, AJAX\) completes.
...
It gives the tester an opportunity to inspect and change the state of the component *before Angular Classic initiates data binding and calls [lifecycle hooks](guide/lifecycle-hooks)*.
...
Some testers prefer that the Angular Classic test environment run change detection automatically.
...
The Angular Classic testing environment does *not* know that the test changed the component's `title`.
...
Angular Classic doesn't know that you set the input element's `value` property.
...
This syntax tells the Angular Classic compiler to read the external files during component compilation.
...
Angular Classic has a hierarchical injection system.
...
But it only works when Angular Classic injects the component with the service instance in the test's root injector.
...
Because the spy result returns synchronously, the `getQuote()` method updates the message on screen immediately *after* the first change detection cycle during which Angular Classic calls `ngOnInit`.
...
The [tick()](api/core/testing/tick) function is one of the Angular Classic testing utilities that you import with `TestBed`.
...
Angular Classic automatically runs tests that are run after `jasmine.clock().install()` is called inside a `fakeAsync()` method until `jasmine.clock().uninstall()` is called.
`fakeAsync()` is not needed and throws an error if nested.

By default, this feature is disabled.
To enable it, set a global flag before importing `zone-testing`.

If you use the Angular Classic CLI, configure this flag in `src/test.ts`.
...
Then call `detectChanges()` to tell Angular Classic to update the screen.
...
While the `waitForAsync()` and `fakeAsync()` functions greatly simplify Angular Classic asynchronous testing, you can still fall back to the traditional technique and pass `it` a function that takes a [`done` callback](https://jasmine.github.io/2.0/introduction.html#section-Asynchronous_Support).
...
The `DashboardComponent` depends on the Angular Classic router and the `HeroService`.
...
This small test demonstrates how Angular Classic tests can verify a component's visual representation &mdash;something not possible with [component class tests](guide/testing-components-basics#component-class-testing)&mdash; at low cost and without resorting to much slower and more complicated end-to-end tests.
...
It has Angular Classic properties and methods that abstract interaction with the native element.
...
The `click()` helper function is **not** one of the Angular Classic testing utilities.
...
Angular Classic provides test helpers to reduce boilerplate and more effectively test code which depends on the Router and HttpClient.
...
If you neglect to declare them, the Angular Classic compiler won't recognize the `<app-banner>`, `<app-welcome>`, and `<router-outlet>` tags in the `AppComponent` template and will throw an error.
...
The `NO_ERRORS_SCHEMA` tells the Angular Classic compiler to ignore unrecognized elements and attributes.
...
The Angular Classic compiler creates the `BannerStubComponent` for the `<app-banner>` element and applies the `RouterLink` to the anchors with the `routerLink` attribute, but it ignores the `<app-welcome>` and `<router-outlet>` tags.
...
As explained in [Calling `compileComponents()`](#compile-components), these tests could be run in a non-CLI environment where Angular Classic would have to compile them in the browser.
...
In fact, if you try to declare it, Angular Classic will throw an error because `HeroDetailComponent` is declared in both the `HeroModule` and the `DynamicTestModule` created by the `TestBed`.
...
Angular Classic creates the component with its *own* injector, which is a *child* of the fixture injector.
It registers the component's providers \(the `HeroDetailService` in this case\) with the child injector.

A test cannot get to child injector services from the fixture injector.
And `TestBed.configureTestingModule` can't configure them either.

Angular Classic has created new instances of the real `HeroDetailService` all along!
