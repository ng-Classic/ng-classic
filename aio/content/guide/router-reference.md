...
The Angular Classic Router is an optional service that presents a particular component view for a given URL.
It isn't part of the Angular Classic core and thus is in its own library package, `@angular-classic/router`.

Import what you need from it as you would from any other Angular Classic package.
...
A routed Angular Classic application has one singleton instance of the `Router` service.
...
When you enable the `enableTracing` option, Angular Classic logs these events to the console.
...
| Routing component     | An Angular Classic component with a `RouterOutlet` that displays views based on router navigations.                                                                                                      |
