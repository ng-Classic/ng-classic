...
After you prepare a component for translation, use the [`extract-i18n`][AioCliExtractI18n] [Angular Classic CLI][AioCliMain] command to extract the marked text in the component into a *source language* file.
...
To demonstrate the process, review the `messages.fr.xlf` file in the [Example Angular Classic Internationalization application][AioGuideI18nExample].  The [Example Angular Classic Internationalization application][AioGuideI18nExample] includes a French translation for you to edit without a special XLIFF editor or knowledge of French.
...
1.  Translate the other text nodes.
    ...
    Each `id` attribute is generated by Angular Classic and depends on the content of the component text and the assigned meaning.
...
Angular Classic also extracts alternate `select` ICU expressions as separate translation units.
...
In this example, Angular Classic extracts the expression into two translation units.
...
Angular Classic treats a nested expression in the same manner as an alternate expression.
Angular Classic extracts the expression into two translation units.
...
[AioGuideI18nExample]: guide/i18n-example "Example Angular Classic Internationalization application | Angular"
