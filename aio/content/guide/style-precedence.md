...
When there are multiple bindings to the same class name or style attribute, Angular Classic uses a set of precedence rules to determine which classes or styles to apply to the element.
...
If `dirWithHostBinding` sets its binding to `null`, Angular Classic removes the `width` property entirely.
