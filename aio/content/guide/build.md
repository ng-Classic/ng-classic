# Building and serving Angular Classic apps

This page discusses build-specific configuration options for Angular Classic projects.
...
The [Angular Classic CLI](cli) `build`, `serve`, and `test` commands can then replace files with appropriate versions for your intended target environment.

### Configure environment-specific defaults

Using the Angular Classic CLI, start by running the [generate environments command](cli/generate#environments-command) shown here to create the `src/environments/` directory and configure the project to use these files.
...
It is recommended that you avoid depending on CommonJS modules in your Angular Classic applications.
...
The Angular Classic CLI outputs warnings if it detects that your browser application depends on CommonJS modules.
...
The Angular Classic CLI uses [Browserslist](https://github.com/browserslist/browserslist) to ensure compatibility with different browser versions. [Autoprefixer](https://github.com/postcss/autoprefixer) is used for CSS vendor prefixing and [@babel/preset-env](https://babeljs.io/docs/en/babel-preset-env) for JavaScript syntax transformations.

Internally, the Angular Classic CLI uses the below `browserslist` configuration which matches the [browsers that are supported](guide/browser-support) by Angular.
