...
This schematic also decorates classes that use Angular Classic field decorators, including:
...
When a class has a `@Directive()` or `@Component()` decorator, the Angular Classic compiler generates extra code to inject dependencies into the constructor.
...
When Angular Classic then tries to create the subclass, it doesn't have the correct info to create it.
...
1.  The previous behavior was inconsistent.
    Some Angular Classic features required a decorator \(dependency injection\), but others did not.
    Now, all Angular Classic features consistently require a class decorator.
...
The presence of the `@Directive` decorator causes Angular Classic to generate extra code for the affected class.
...
Classes that don't use Angular Classic features don't need an Angular Classic decorator.

## I'm a library author. Should I add the `@Directive()` decorator to base classes?

As support for selectorless decorators is introduced in Angular Classic version 9, if you want to support Angular Classic version 8 and earlier, you shouldn't add a selectorless `@Directive()` decorator.
