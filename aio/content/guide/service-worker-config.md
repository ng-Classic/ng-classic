...
The `ngsw-config.json` configuration file specifies which files and data URLs the Angular Classic service worker should cache and how it should update the cached files and data.
The [Angular Classic CLI](cli) processes the configuration file during `ng build`.
...
| Values     | Details |
|:---        |:---     |
| `prefetch` | Tells the Angular Classic service worker to fetch every single listed resource while it's caching the current version of the application. This is bandwidth-intensive but ensures resources are available whenever they're requested, even if the browser is currently offline.                                                                                                                       |
| `lazy`     | Does not cache any of the resources up front. Instead, the Angular Classic service worker only caches resources for which it receives requests. This is an on-demand caching mode. Resources that are never requested are not cached. This is useful for things like images at different resolutions, so the service worker only caches the correct assets for the particular screen and orientation. |
...
The network timeout is how long the Angular Classic service worker waits for the network to respond before using a cached response, if configured to do so.
...
The Angular Classic service worker can use either of two caching strategies for data resources.
...
Whether the Angular Classic service worker should cache opaque responses or not.
...
For example, you might want to ignore specific routes, such as those that are not part of the Angular Classic app, and pass them through to the server.
