...
A pull request is how you share your update in a way that allows it to be merged it into the `ng-angular/angular` repo.
...
A pull request shares a branch in `personal/angular`, your forked repo, with the `ng-angular/angular` repo.
After your pull request is approved and merged, the new commits from your branch are added to the main branch in the `ng-angular/angular` repo.
The commits in your branch, and their messages, become part of the `ng-angular/angular` repo.
...
1.  Your commit messages become part of the documentation of the changes made to Angular.
    Because they become part of the `ng-angular/angular` repo, they must conform to a specific format so that they are easy to read.
...
1.  You might need to squash the commits that you made while developing your update.
    It's normal to save your changes as intermediate commits while you're developing a large update, but your pull request represents only one change to the `ng-angular/angular` repo.
    Squashing the commits from your working branch into fewer, or just one commit, makes the commits in your pull request match the changes your update makes to the `ng-angular/angular` repo.

## Format commit messages for a pull request

Commits merged to `ng-angular/angular` must have messages that are correctly formatted.
...
For the complete specification of Angular Classiccommit messages, see [Commit message format](https://github.com/ng-angular/angular/blob/main/CONTRIBUTING.md#commit).
...
You want to sync the `main` branch of your origin repo with the `main` branch of the upstream `ng-angular/angular` before you open a pull request.
...
1.  In the output of the previous `git log` command, find the entry that contains `upstream/main`.
    ...
    1.  **Confirm that the entry that contains `upstream/main` also contains `origin/main` and `main`**
        ...
        If it doesn't, you must resync your clone and your fork of `ng-angular/angular`, and then rebase the branch before you continue.
