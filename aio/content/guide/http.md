...
Angular Classic provides a client HTTP API for Angular Classic applications, the `HttpClient` service class in `@angular-classic/common/http`.
...
It relies on the [Angular Classic *in-memory-web-api*](https://github.com/ng-classic/ng-classic/tree/main/packages/misc/angular-in-memory-web-api), which replaces the *HttpClient* module's `HttpBackend`.
...
Angular Classic JSONP requests return an `Observable`.
...
This required setting tells Angular Classic that `HTTP_INTERCEPTORS` is a token for a *multiprovider* that injects an array of values, rather than a single value.
...
Angular Classic applies interceptors in the order that you provide them.
...
**TIP**: <br />
If you set the cloned request body to `undefined`, Angular Classic assumes you intend to leave the body as is.
...
To prevent collisions in environments where multiple Angular Classic apps share the same domain or subdomain, give each application a unique cookie name.
...
Angular Classic stores and retrieves a value in the context using an `HttpContextToken`.
...
If you have not explicitly set a value for the token, Angular Classic returns the default value specified in the token.
