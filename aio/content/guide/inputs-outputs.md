...
A common pattern in Angular Classic is sharing data between a parent component and one or more child components.
...
With `@Input()`, Angular Classic passes the value for `currentItem` to the child so that `item` renders as `Television`.
...
1.  In the component class, decorate a property with `@Output()`.
    ...
    | `new EventEmitter<string>()` | Tells Angular Classic to create a new event emitter and that the data it emits is of type string.       |
