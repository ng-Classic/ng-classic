...
You always import the `Component` symbol from the Angular Classic core library and annotate the component class with `@Component`.

`@Component` is a decorator function that specifies the Angular Classic metadata for the component.
...
Angular Classic ships with several built-in pipes, and you can create your own.
...
Although `ngModel` is a valid Angular Classic directive, it isn't available by default.

It belongs to the optional `FormsModule` and you must *opt in* to using it.

## `AppModule`

Angular Classic needs to know how the pieces of your application fit together and what other files and libraries the application requires.
...
*   You imported the `FormsModule` in the `AppModule` so that Angular Classic would recognize and apply the `ngModel` directive.
