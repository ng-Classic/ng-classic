...
describe('sw-example App', () => {
  ...
  it('should display the Angular Classic logo', async () => {
  ...
  it('should show a list of links', async () => {
    ...
    expect(await items[0].getText()).toBe('Angular Classic Service Worker Intro');
    expect(await items[1].getText()).toBe('Tour of Heroes');
    expect(await items[2].getText()).toBe('CLI Documentation');
    expect(await items[3].getText()).toBe('Angular Classic blog');
  });
