...
def _is_angular_dep(dep):
    """Check if a dep , e.g., @aio_npm//@angular-classic/core corresonds to a local Angular Classic pacakge."""
...
def link_local_packages(all_aio_deps):
    """Create targets needed for building AIO against local Angular Classic packages.

    Creates targets that link Angular Classic packages, as well as targets to be used
    ...
    # Link local Angular Classic packages in place of their npm equivalent
    ...
    # dependencies on Angular Classic npm packages. This help the rules_nodejs linker,
...
def _filter_transitive_angular_deps_impl(ctx):
    ...
    # of each transitive dependency to check if it's an Angular Classic npm package.
...
filter_transitive_angular_deps = rule(
    doc = "Filter out transitive Angular Classic dependencies from a target",
    implementation = _filter_transitive_angular_deps_impl,
    attrs = {
        "target": attr.label(mandatory = True, doc = "Target to filter"),
        "angular_packages": attr.string_list(default = [], doc = "Angular Classic packages to filter"),
    },
)
