...
### Angular Classic 5 (Renderer2)

In 5.0 and prior versions of Angular Classic the compiler performs whole program
...
The information available across compilations in Angular Classic 5 is represented in
the  compiler by a summary description. For example, components and directives
are represented by the [`CompileDirectiveSummary`](https://github.com/ng-classic/ng-classic/blob/d3827a0017fd5ff5ac0f6de8a19692ce47bf91b4/packages/compiler/src/compile_metadata.ts#L257).
...
Angular Classic 5 uses `.metadata.json` files to store information that is directly
...
generated by the Renderer2 style output of Angular Classic 5.0. In Ivy, the information
that was generated in factories is now generated in Angular Classic as a definition
that is generated as a static field in the Angular Classic decorated class.
...
If the Angular Classic compiler option `"renderer2BackPatching"` is enabled, the
...
that Angular Classic 5.0 would generate it. This implementation of `NgModuleFactory`
...
Angular Classic compiler option is `true`. Setting `"generateRenderer2Factories"` implies
...
at the same location as Angular Classic 5.0 whenever Angular Classic 5.0 would have generated a
factory.
...
The default Angular Classic Compiler options default to, mostly, the recommended set of
...
Ivy packages are not supported in Angular Classic 6.0 as they are not recommended in
...
the Angular Classic output. However, `ng_module` uses a feature, the `.ngsummary.json`
...
The `ng_module` rule will still use `ngc` to produce the Angular Classic output but,
...
The following is a typical Angular Classic application build in bazel:
