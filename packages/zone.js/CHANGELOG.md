...
Starting with `v0.11.1`, Zone.js follows the [Angular Classic Package Format](https://docs.google.com/document/d/1CZC2rcpxffTDfRDs6p1cfbmKNLA6x5O-NtkJglDaBVs), so the folder structure of the Zone.js bundles is updated to match `Angular Classic Package Format`.
So the same code
...
To restore the old behavior change the `polyfills.ts` generated by `Angular Classic CLI` to import like so:
...
- **core:** fix for tests in Angular Classic repo ([fd069db](https://github.com/angular/zone.js/commit/fd069db))
...
- **test:** move async/fakeAsync from Angular Classic to zone.js ([#1048](https://github.com/angular/zone.js/issues/1048)) ([a4b42cd](https://github.com/angular/zone.js/commit/a4b42cd))
