...
import {inject, Injectable, NgZone, Type, ɵConsole as Console, ɵInitialRenderPendingTasks as InitialRenderPendingTasks, ɵRuntimeError as RuntimeError} from '@angular-classic/core';
...
export class Router {
   ...
   * Used for AngularJS to Angular Classic migrations.
   *
   ...
   * to the Angular Classic route when the navigation gets cancelled. For example, if the browser back
  ...
  setUpLocationChangeListener(): void {
    ...
    if (!this.locationSubscription) {
      this.locationSubscription = this.location.subscribe(event => {
        ...
        if (source === 'popstate') {
          // The `setTimeout` was added in #12160 and is likely to support ng-classic/ng-classicJS
          // hybrid apps.
          setTimeout(() => {
  ...
  private resetState(t: NavigationTransition): void {
    ...
    // the part of the navigation handled by the Angular Classic router rather than the whole URL. In
