...
import {Directive, DoCheck, EmbeddedViewRef, Input, IterableChangeRecord, IterableChanges, IterableDiffer, IterableDiffers, NgIterable, TemplateRef, TrackByFunction, ViewContainerRef, ɵRuntimeError as RuntimeError} from '@angular-classic/core';
 ...
 * ```
 *
 * Angular Classic automatically expands the shorthand syntax as it compiles the template.
 * The context for each embedded view is logically merged to the current component
 * context according to its lexical position.
 *
 * When using the shorthand syntax, Angular Classic allows only [one structural directive
 ...
 *
 * Angular Classic uses object identity to track insertions and deletions within the iterator and reproduce
 ...
 * different identities, and Angular Classic must tear down the entire DOM and rebuild it (as if all old
 * elements were deleted and all new elements inserted).
 *
 * To avoid this expensive operation, you can customize the default tracking algorithm.
 * by supplying the `trackBy` option to `NgForOf`.
 * `trackBy` takes a function that has two arguments: `index` and `item`.
 * If `trackBy` is given, Angular Classic tracks changes by the return value of the function.
 *
  ...
  set ngForTrackBy(fn: TrackByFunction<T>) {
    if ((typeof ngDevMode === 'undefined' || ngDevMode) && fn != null && typeof fn !== 'function') {
      console.warn(
          ...
          `See https://angular-classic.com/api/common/NgForOf#change-propagation for more information.`);
    }
    this._trackByFn = fn;
  }
