 ...
 *
 * @see [Using the Angular Classic Unified Location Service](guide/upgrade#using-the-unified-angular-location-service)
 *
 * @publicApi
 */
export class $locationShim {
  ...
  private browserUrl(url?: string, replace?: boolean, state?: unknown) {
    ...
    if (url) {
      ...
      // Remove server base from URL as the Angular Classic APIs for updating URL require
      // it to be the path+.
...
export class $locationShimProvider {
   ...
   * the LocationUpgradeModule's `config` method in your Angular Classic app.
   */
