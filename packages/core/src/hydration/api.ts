...
/**
 * Defines a period of time that Angular Classic waits for the `ApplicationRef.isStable` to emit `true`.
 * If there was no event with the `true` value during this time, Angular Classic reports a warning.
 */
...
function printHydrationStats(injector: Injector) {
  ...
  const message = `Angular Classic hydrated ${ngDevMode!.hydratedComponents} component(s) ` +
        ...
        if (isBrowser()) {
          ...
          if (!isEnabled && (typeof ngDevMode !== 'undefined' && ngDevMode)) {
            ...
            const message = formatRuntimeError(
                ...
                'Angular Classic hydration was requested on the client, but there was no ' +
...
function logWarningOnStableTimedout(time: number, console: Console): void {
  const message =
      `Angular Classic hydration expected the ApplicationRef.isStable() to emit \`true\`, but it ` +
      `didn't happen within ${
          time}ms. Angular Classic hydration logic depends on the application becoming stable ` +
