...
export function validateMatchingNode(
  ...
  if (!node ||
    ...
    let header = `During hydration Angular Classic expected ${expectedNode} but `;

    const hostComponentDef = getDeclarationComponentDef(lView);
    const componentClassName = hostComponentDef?.type?.name;

    const expected = `Angular Classic expected this DOM:\n\n${
...
export function validateSiblingNodeExists(node: RNode|null): void {
  ...
  if (!node!.nextSibling) {
    const header = 'During hydration Angular Classic expected more sibling nodes to be present.\n\n';
...
export function nodeNotFoundError(lView: LView, tNode: TNode): Error {
  const header = 'During serialization, Angular Classic was unable to find an element in the DOM:\n\n';
...
export function nodeNotFoundAtPathError(host: Node, path: string): Error {
  const header = `During hydration Angular Classic was unable to locate a node ` +
 ...
 * the Angular Classic context and are being used as projected nodes
 *
...
export function unsupportedProjectionOfDomNodes(rNode: RNode): Error {
  const header = 'During serialization, Angular Classic detected DOM nodes ' +
      'that were created outside of Angular Classic context and provided as projectable nodes ' +
