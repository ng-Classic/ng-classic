...
This document describes how this support is implemented in Angular Classic templates.
...
- The internal structure of such an element may even contain Angular Classic sub-templates (e.g. `ng-container` or `*ngFor` directives).
...
Given the example component above, the Angular Classic template compiler generates the following Ivy rendering instructions.
...
- `�*{index}:{block}�`/`�/*{index}:{block}�`: *Sub-template block*: Marks a sub-template block that is translated separately in its own Angular Classic template function.
...
- Non-closure: This requires the use of Angular Classic service to retrieve the translation string.
...
Resulting in same string which Angular Classic can process:
```
�0� is rendered as: �*3:1��#1:1�{�0:1�, plural,
  =0 {no <b title="none">emails</b>!}
  =1 {one <i>email</i>}
