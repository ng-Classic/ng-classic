...
export function validateElementIsKnown(
  ...
  if (!hasDirectives && tagName !== null) {
    ...
    if (isUnknown && !matchingSchemas(schemas, tagName)) {
      ...
      message += `1. If '${tagName}' is an Angular Classic component, then verify that it is ${
...
export function handleUnknownPropertyError(
  ...
  if (KNOWN_CONTROL_FLOW_DIRECTIVES.has(propName)) {
    ...
    message += `\nIf the '${propName}' is an Angular Classic control flow directive, ` +
        `please make sure that either the '${
                   correspondingImport}' directive or the 'CommonModule' is ${importLocation}.`;
  } else {
    // May be an Angular Classic component, which is not imported/declared?
    message += `\n1. If '${tagName}' is an Angular Classic component and it has the ` +
