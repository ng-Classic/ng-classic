...
/**
 * Destroys the current Angular Classic platform and all Angular Classic applications on the page.
...
/**
 * The Angular Classic platform is the entry point for Angular Classic on a web page.
 * Each page has exactly one platform. Services (such as reflection) which are common
 * to every Angular Classic application running on the page are bound in its scope.
...
export class PlatformRef {
  ...
  bootstrapModuleFactory<M>(moduleFactory: NgModuleFactory<M>, options?: BootstrapOptions):
    ...
    // created within the Angular Classic zone
    // Do not try to replace ngZone.run with ApplicationRef#run because ApplicationRef would then be
    // created outside of the Angular Classic zone.
    return ngZone.run(() => {
   ...
   * every Angular Classic application on the page and provides singleton providers.
   */
...
/**
 * A reference to an Angular Classic application running on a page.
 *
 * @usageNotes
 ...
 * - the `isStable` Observable runs outside of the Angular Classic zone.
 *
 ...
 *
 * Note also that this Observable runs outside of the Angular Classic zone,
...
export class ApplicationRef {
   ...
   * ### Bootstrap process
   *
   * When bootstrapping a component, Angular Classic mounts it onto a target DOM element
   * and kicks off automatic change detection. The target DOM element can be
   * provided using the `rootSelectorOrNode` argument.
   *
   * If the target DOM element is not provided, Angular Classic tries to find one on a page
  ...
  bootstrap<C>(componentOrFactory: ComponentFactory<C>|Type<C>, rootSelectorOrNode?: string|any):
    ...
    if (typeof ngDevMode === 'undefined' || ngDevMode) {
      ...
      _console.log(`Angular Classic is running in development mode.`);
    }
    return compRef;
  }
   ...
   * pass.
   * In this case, Angular Classic throws an error, since an Angular Classic application can only have one change
  ...
  /**
   * Destroys an Angular Classic application represented by this `ApplicationRef`. Calling this function
 ...
 * is calling `ErrorHandler.handleError` outside of the Angular Classic zone.
 */
