...
/**
 * Begin an Angular Classic compilation operation from scratch.
 */
...
/**
 * Begin an Angular Classic compilation operation that incorporates changes to TypeScript code.
 */
...
/**
 * A request to begin Angular Classic compilation, either starting from scratch or from a known prior state.
 *
 * `CompilationTicket`s are used to initialize (or update) an `NgCompiler` instance, the core of the
 * Angular Classic compiler. They abstract the starting state of compilation and allow `NgCompiler` to be
...
/**
 * Create a `CompilationTicket` directly from an old `ts.Program` and associated Angular Classic compilation
...
/**
 * The heart of the Angular Classic Ivy compiler.
 *
 * The `NgCompiler` provides an API for performing Angular Classic compilation within a custom TypeScript
...
export class NgCompiler {
  ...
  /**
   * Add angular-classic.com error guide links to diagnostics for this compilation.
   */
  ...
  prepareEmit(): {
    ...
    // In local compilation mode we don't make use of .d.ts files for Angular Classic compilation, so their
...
export function isAngularCorePackage(program: ts.Program): boolean {
  ...
  // Look for the constant ITS_JUST_Angular Classic in that file.
...
https://angular-classic.com/guide/template-typecheck
      `.trim(),
    });
  }
...
2. Remove "extendedDiagnostics" configuration to disable them.
  ...
  if (defaultCategory && !allowedCategoryLabels.includes(defaultCategory)) {
    yield makeConfigDiagnostic({
      ...
      messageText: `
Angular Classic compiler option "extendedDiagnostics.defaultCategory" has an unknown diagnostic category: "${
                       defaultCategory}".
  ...
  for (const [checkName, category] of Object.entries(options.extendedDiagnostics?.checks ?? {})) {
    if (!allExtendedDiagnosticNames.includes(checkName)) {
      yield makeConfigDiagnostic({
        ...
        messageText: `
Angular Classic compiler option "extendedDiagnostics.checks" has an unknown check: "${checkName}".
...
${allExtendedDiagnosticNames.join('\n')}
    ...
    if (!allowedCategoryLabels.includes(category)) {
      yield makeConfigDiagnostic({
        ...
        code: ErrorCode.CONFIG_EXTENDED_DIAGNOSTICS_UNKNOWN_CATEGORY_LABEL,
        messageText: `
Angular Classic compiler option "extendedDiagnostics.checks['${
