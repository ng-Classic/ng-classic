...
import {afterRender, ApplicationRef, Component, ComponentRef, createComponent, destroyPlatform, Directive, ElementRef, EnvironmentInjector, ErrorHandler, getPlatform, inject, Injectable, Input, NgZone, PLATFORM_ID, Provider, TemplateRef, Type, ViewChild, ViewContainerRef, ViewEncapsulation, ɵsetDocument} from '@angular-classic/core';
import {Console} from '@angular-classic/core/src/console';
import {SSR_CONTENT_INTEGRITY_MARKER} from '@angular-classic/core/src/hydration/utils';
  ...
  describe('hydration', () => {
    ...
    describe('public API', () => {
      it('should allow to disable DOM hydration using `withNoDomReuse` feature', async () => {
        ...
        verifyHasNoLog(appRef, 'Angular Classic hydrated');
    ...
    describe('hydration', () => {
      ...
      describe('basic scenarios', () => {
        ...
        it('should support text and HTML elements in nested components', async () => {
          ...
          verifyHasNoLog(
              appRef,
              'NG05000: Angular Classic detected that hydration was enabled for an application ' +
    ...
    describe('ngSkipHydration', () => {
      ...
      it('should skip hydrating when ng-containers exist and ngSkipHydration attribute is present',
         async () => {
           ...
           verifyHasLog(
               appRef,
               'Angular Classic hydrated 1 component(s) and 6 node(s), 1 component(s) were skipped');
    ...
    describe('content projection', () => {
      it('should project plain text', async () => {
        ...
        verifyHasLog(
            appRef, 'Angular Classic hydrated 2 component(s) and 5 node(s), 0 component(s) were skipped');
      ...
      it('should throw an error when projecting DOM nodes via ViewContainerRef.createComponent API',
         async () => {
           ...
           try {
             ...
             expect(errorMessage)
                 .toContain(
                     'During serialization, Angular Classic detected DOM nodes that ' +
                     'were created outside of Angular Classic context');
    ...
    `,
        ...
        await hydrate(html, SimpleComponent, withNoopErrorHandler()).catch((err: unknown) => {
          ...
          expect(message).toContain(
              'During hydration Angular Classic expected a text node but found <span>');
    ...
    `,
        ...
        await hydrate(html, SimpleComponent, withNoopErrorHandler()).catch((err: unknown) => {
          const message = (err as Error).message;
          expect(message).toContain(
              'During hydration Angular Classic expected <div> but the node was not found');
    ...
    `,
        ...
        await hydrate(html, SimpleComponent, withNoopErrorHandler()).catch((err: unknown) => {
          const message = (err as Error).message;
          expect(message).toContain('During hydration Angular Classic expected <b> but found <span>');
      ...
      `,
        ...
        await hydrate(html, SimpleComponent, withNoopErrorHandler()).catch((err: unknown) => {
          const message = (err as Error).message;
          expect(message).toContain(
              'During hydration Angular Classic expected a comment node but found <span>');
      ...
      it('should handle sibling count mismatch', async () => {
        ...
        await hydrate(html, SimpleComponent, withNoopErrorHandler()).catch((err: unknown) => {
          const message = (err as Error).message;
          expect(message).toContain(
              'During hydration Angular Classic expected more sibling nodes to be present');
      ...
      `,
        ...
        await ssr(SimpleComponent, undefined, withNoopErrorHandler()).catch((err: unknown) => {
          const message = (err as Error).message;
          expect(message).toContain(
              'During serialization, Angular Classic was unable to find an element in the DOM');
          expect(message).toContain('<b>…</b>  <-- AT THIS LOCATION');
        });
      });
      ...
      `,
        ...
        await hydrate(html, SimpleComponent, withNoopErrorHandler()).catch((err: unknown) => {
          const message = (err as Error).message;
          expect(message).toContain(
              'During hydration Angular Classic was unable to locate a node using the "firstChild" path, ' +
      ...
      it('should handle a case when a node is not found (invalid DOM)', async () => {
        ...
        try {
          ...
          expect(message).toContain(
              'During hydration, Angular Classic expected an element to be present at this location.');
          expect(message).toContain('<!-- container -->  <-- AT THIS LOCATION');
      ...
      it('should log an warning when there was no hydration info in the TransferState',
         async () => {
           ...
           verifyHasLog(
               appRef,
               'NG0505: Angular Classic hydration was requested on the client, ' +
