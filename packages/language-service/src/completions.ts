...
export class CompletionBuilder<N extends TmplAstNode|AST> {
  ...
  private getGlobalPropertyExpressionCompletion(
    ...
    return {
      entries,
      // Although this completion is "global" in the sense of an Angular Classic expression (there is no
      // explicit receiver), it is not "global" in a TypeScript sense since Angular Classic expressions have
  ...
  private getElementTagCompletion(this: CompletionBuilder<TmplAstElement|TmplAstText>):
    ...
    // Don't provide non-Angular Classic tags (directive === null) because we expect other extensions (i.e.
