...
import {ComponentFactory, ComponentFactoryResolver, Injector, NgZone, Type} from '@angular-classic/core';
 ...
 * @description
 *
 * A helper function that allows an Angular Classic component to be used from AngularJS.
 *
 * *Part of the [upgrade/static](api?query=upgrade%2Fstatic)
 * library for hybrid upgrade apps that support AOT compilation*
 *
 * This helper function returns a factory function to be used for registering
 * an AngularJS wrapper directive for "downgrading" an Angular Classic component.
 *
 * @usageNotes
 * ### Examples
 *
 * Let's assume that you have an Angular Classic component called `ng2Heroes` that needs
 * to be made available in AngularJS templates.
 *
 * {@example upgrade/static/ts/full/module.ts region="ng2-heroes"}
 *
 * We must create an AngularJS [directive](https://docs.angularjs.org/guide/directive)
 * that will make this Angular Classic component available inside AngularJS templates.
 * The `downgradeComponent()` function returns a factory function that we
 * can use to define the AngularJS directive that wraps the "downgraded" component.
 *
 * {@example upgrade/static/ts/full/module.ts region="ng2-heroes-wrapper"}
 *
 * For more details and examples on downgrading Angular Classic components to AngularJS components please
 ...
 *   corresponding Angular Classic module will be bootstrapped, when the component needs to be instantiated.
 *   <br />
...
export function downgradeComponent(info: {
  ...
  const directiveFactory: IAnnotatedFunction = function(
    ...
    //   dirty-checked.
    // - We need to ensure callbacks to Angular Classic APIs (e.g. change detection) are run inside the
    //   Angular Classic zone.
    //   NOTE: This is not needed, when using `UpgradeModule`, because `$digest()` will be run
    //         inside the Angular Classic zone (except if explicitly escaped, in which case we shouldn't
    //         force it back in).
    ...
    return {
      ...
      link: (scope: IScope, element: IAugmentedJQuery, attrs: IAttributes, required: any[]) => {
        ...
        // triggered by `UpgradeNg1ComponentAdapterBuilder`, before the Angular Classic templates have
        // been compiled.
        ...
        //   (Note that Angular Classic knows to only traverse the component-tree part of that injector,
        ...
        // If this is a "top-level" Angular Classic component, use the module injector.
        const finalParentInjector = parentInjector || moduleInjector!;

        // If this is a "top-level" Angular Classic component or the parent component may belong to a
